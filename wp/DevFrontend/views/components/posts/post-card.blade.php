@mock('post_card')
@if (!isset($data))
    {{ debug($data ?? 'null: DATA post_card') }}
@else
        ...
        <div class="col-12 col-lg-6">
            <div class="card border-0">
                ...
                <div class="card-body ps-4">
                    <h1 class="fw-bold" style="font-size: calc(1rem + 0.5vw);">
                        {{ strtoupper($data['title'] ?? 'null: title') }}</h1>

                    {{-- <h1 class="card-title py-2">{{ strtoupper($data['name']) }}</h1> --}}
                    <p class="card-text" style="text-indent: 20px;">{{ $data['excerpt'] ?? 'null: excerpt' }}</p>
                    <a href="/fe/category/thung-rac" class="btn p-0 m-2 border-0">
                        <span class="badge text-bg-light">từ khóa 1</span>
                    </a>
                    <a href="/fe/category/thung-rac" class="btn p-0 m-2 border-0">
                        <span class="badge text-bg-light">từ khóa 2</span>
                    </a>
                </div>
            </div>
        </div>
